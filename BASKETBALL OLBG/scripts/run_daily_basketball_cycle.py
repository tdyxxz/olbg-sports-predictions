...
STRATEGIES = [
    ...
    {
        "name": "shadow_favorites_v1",
        "args": ["--config", ".\\shadow_favorites_v1.json"],
    },
]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run daily basketball prediction cycle and refresh settlement.")
    ...
    failures = 0
    olbg_proc = run_step([str(ROOT_SCRIPTS_DIR / "fetch_olbg_event_board.py"), "--sport", "basketball"])
    ...
    for strategy in STRATEGIES:
        cmd = [str(SCRIPTS_DIR / "predict_nba_card.py"), "--date", args.date, *strategy["args"]]
        if args.fast:
            cmd.append("--fast")
        proc = run_step(cmd)
