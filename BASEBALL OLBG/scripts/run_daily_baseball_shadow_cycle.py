...
STRATEGIES = [
    ...
    {
        "name": "shadow_heavy_favorites_v1",
        "args": ["--config", ".\\config\\shadow_heavy_favorites_v1.json"],
    },
    {
        "name": "shadow_favorites_plus_shortdogs_v1",
        "args": ["--config", ".\\config\\shadow_favorites_plus_shortdogs_v1.json"],
    },
]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run baseline + shadow baseball prediction cycle and refresh settlement.")
    ...
    failures = 0
    olbg_proc = run_step([str(ROOT_SCRIPTS_DIR / "fetch_olbg_event_board.py"), "--sport", "baseball"])
    ...
    for strategy in STRATEGIES:
        cmd = [str(SCRIPTS_DIR / "predict_mlb_card.py"), "--date", args.date, *strategy["args"]]
        if args.fast:
            cmd.append("--fast")
        proc = run_step(cmd)
        print(f"[{strategy['name']}] returncode={proc.returncode}")
